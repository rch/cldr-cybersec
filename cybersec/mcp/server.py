...
@mcp.tool()
async def bootstrap_debug_pyflink() -> dict:
    """Gather diagnostic information for PyFlink job failures.

    Collects:
    - Python environment (version, path, PYTHONPATH)
    - PyFlink installation status
    - Flink configuration
    - Job submission logs
    - TaskManager logs (last errors)
    - Platform-specific info (macOS vs Linux)
    ...
    Returns:
        ...
        - environment: Python paths and versions
        - flink_config: Relevant Flink Python settings
        - logs: Recent error logs
        - recommendations: Suggested fixes based on findings
    """
    from ..health.pyflink_diagnostics import gather_pyflink_diagnostics
    return await gather_pyflink_diagnostics()
